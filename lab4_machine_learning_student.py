...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    average = float(sum(IDandRatingsTuple[1]))/count
    
    return (IDandRatingsTuple[0], (count, average))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (k, v): (v[1][1], v[0], v[1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    testRDD = squaredErrorsRDD = (predictedReformattedRDD
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda (k, x): math.pow(x[0]-x[1], 2)))
...
sumRatings = trainingRDD.map(lambda x:x[2]).reduce(lambda x,y:x+y)
countRatings = trainingRDD.count()

trainingAvgRating = float(sumRatings)/countRatings
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x:x[0] not in myRatedMovies[1])
