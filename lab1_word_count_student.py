...
wordsGrouped = wordPairs.groupByKey().mapValues(lambda x : x)
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = len(wordPairs.groupByKey().map(lambda (k,v) : k).collect())
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    wordPair = wordListRDD.map(lambda x : (x, 1))
...
def removePunctuation(text):
    ...
    """
    text = re.sub('[^a-z1-9A-Z ]+', '', text).strip().lower()
    return text
