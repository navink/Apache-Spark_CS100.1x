...
uniqueHostCount = len(uniqueHosts.map(lambda (x, y) : x).collect())
...
dayAndHosts = map(list, zip(*(dailyHosts.collect())))
daysWithHosts = dayAndHosts[0]
hosts = dayAndHosts[1]
...
# TODO: Replace <FILL IN> with appropriate code
daysWithAvgTuple = map(list, zip(*(avgDailyReqPerHost.collect())))
daysWithAvg = daysWithAvgTuple[0]
avgs = daysWithAvgTuple[1]
...
errHostsTop25 = errHostsSum.takeOrdered(25, lambda (a, b): -b)
...
# TODO: Replace <FILL IN> with appropriate code
daysAndErrors404 = map(list, zip(*(errDateSorted.collect())))
daysWithErrors404 = daysAndErrors404[0]
errors404ByDay = daysAndErrors404[1]
...
topErrDate = errDateSorted.takeOrdered(5, lambda (x, y) : -y)
...
# TODO: Replace <FILL IN> with appropriate code
hoursAndErrors404 = map(list, zip(*(hourRecordsSorted.collect())))

hoursWithErrors404 = hoursAndErrors404[0]
errors404ByHours = hoursAndErrors404[1]
