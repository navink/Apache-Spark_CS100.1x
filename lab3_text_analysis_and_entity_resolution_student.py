...
def simpleTokenize(string):
    ...
    """
    return [s.lower() for s in re.split(split_regex, string) if s != '']
...
def tokenize(string):
    ...
    """            
    return [s.lower() for s in re.split(split_regex, string) if s != '' and s.lower() not in stopwords]
...
def countTokens(vendorRDD):
    ...
    """    
    VendorIDTokensRDD = vendorRDD.mapValues(lambda v : len(v))
    
    sum = 0
    
    for (k, v) in VendorIDTokensRDD.collect():
        sum = sum + v
...
def findBiggestRecord(vendorRDD):
    ...
    return vendorRDD.takeOrdered(1, lambda (x, y) : -len(y))
...
def tf(tokens):
    ...
    return {key: value/float(numTokens) for (key, value) in tokenDict.iteritems()}
...
def dotprod(a, b):
    ...
    """
    # sum(map(operator.mul, a.values(), b.values()))
    #sum([a[k]*b[k] for k in a.keys() if k in b.keys()])
    
    mults = [a[k]*b[k] for k in a.keys() if k in b.keys()]
    ...
    #return [a[k]*b[k] for k in a.keys() if k in b.keys()]
...
for x in trueDupsRDD.collect():
    sum = sum + x[1][0]

#avgSimDups = sum(trueDupsRDD.collect()[1][0])/trueDupsCount
avgSimDups = sum/trueDupsCount

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard).filter(lambda (x, y) : y[1] == None))
nonDupsCount = nonDupsRDD.count()              

sum = 0
for x in nonDupsRDD.collect():
    sum = sum + x[1][0]
    
#avgSimNon = sum(nonDupsRDD.collect().values())/nonDupsCount
avgSimNon = sum/nonDupsCount
print avgSimNon
...
amazonWeightsRDD = amazonFullRecToToken.mapValues(lambda v : tfidf(v, idfsFullBroadcast.value))
googleWeightsRDD = googleFullRecToToken.mapValues(lambda v : tfidf(v, idfsFullBroadcast.value))
...
# TODO: Replace <FILL IN> with appropriate code
#testVec12 = {'foo': 2, 'bar': 3, 'baz': 5 }
#print norm(testVec12)
#xx = amazonWeightsRDD.collectAsMap()
#print amazonWeightsRDD.take(1)
#print xx['b000ea9u2a']
...
def fastCosineSimilarity(record):
    ...
    #amazonRec = record[0][0][0]    
    #googleRec = record[0][0][1]
    #tokens = record[0][1]
    #print tokens
    
    amz = amazonWeightsBroadcast.value
    goo = googleWeightsBroadcast.value
    
    amzVec = amz[amazonRec]
    gooVec = goo[googleRec]
    
    #print gooVec['includes']
    #print amzVec
    #s = <FILL IN>
    
    s = 0.0
    for token in tokens:        
        if token in amzVec and token in gooVec: 
            s = s + amzVec[token]*gooVec[token]
...
#fastCosineSimilarity(commonTokens.take(1))

#myRec = commonTokens.filter(lambda ((aID, gURL), cs): aID == 'b00005lzly' and gURL == 'http://www.google.com/base/feeds/snippets/13823221823254120257')
#print fastCosineSimilarity(myRec.collect())[1]
